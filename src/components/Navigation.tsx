  ...
  return (
    <aside className="sticky top-0 z-50 hidden h-screen w-[17rem] shrink-0 flex-col border-r border-white/5 bg-[#070912]/70 px-4 py-5 backdrop-blur-2xl md:flex">
      ...
      <NavLink
        ...
        className={({ isActive }) =>
          cn(
            'mb-5 flex w-full items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.035] p-3 text-left transition-all hover:border-accent/25 hover:bg-white/[0.055]',
            isActive && 'border-accent/30 bg-white/[0.06] ring-1 ring-accent/15'
          )
        }
      >
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto pr-1 scrollbar-hide">
        ...
        {navItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                'group relative flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left text-sm font-medium transition-all duration-300',
                isActive
    ...
    <nav className="mobile-bottom-nav fixed bottom-3 left-5 right-5 z-[70] flex h-[4.75rem] items-center justify-between gap-1 overflow-hidden rounded-[1.75rem] border border-white/12 bg-[#070a16]/88 px-2 shadow-2xl shadow-black/50 backdrop-blur-2xl md:hidden">
      ...
      {dockItems.map((item) => (
        <NavLink
          ...
          className={({ isActive }) =>
            cn(
              'relative flex min-w-0 flex-1 touch-manipulation flex-col items-center justify-center gap-1 rounded-2xl px-1 py-2 transition-all duration-200 active:opacity-90',
        ...
        >
          {({ isActive }) => (
            <>
              ...
              <span
                className={cn(
                  'relative z-10 text-[10px] font-semibold leading-none transition-all',
                  isActive ? 'text-white' : ''
                )}
              >
                {item.label}
      ...
      >
        {({ isActive }) => (
          <>
            ...
            <div
              className={cn(
                'relative z-10 flex h-6 w-6 items-center justify-center overflow-hidden rounded-full border transition-all duration-200',
                isActive ? 'scale-110 border-accent' : 'border-white/20'
              )}
            >
              {avatarUrl ? (
                <img src={avatarUrl} alt={displayName} className="h-full w-full object-cover" referrerPolicy="no-referrer" />
            ...
            <span className={cn('relative z-10 text-[10px] font-semibold leading-none transition-all', isActive ? 'text-white' : '')}>
              Profile
            </span>
          </>
        )}
